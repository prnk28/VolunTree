...
-(NSString*)validate{
    ...
    switch (type) {
        case TYPE_EMAIL:
            err = ([self validateEmail:self.text]&&[self emailAvailible]&&![self.text isEqualToString:@""])? @"" : @"Invalid email address";
            break;
        case TYPE_NAME:
        ...
        case TYPE_NUMBER:
            ...
            break;
        case TYPE_USER:
            err = ([self usernameAvailible]&&![self.text isEqualToString:@""])? @"" : @"Invalid username";
            break;
    }
...
- (BOOL)emailAvailible {
    ...
    [query whereKey:@"email" equalTo:self.text];
...
- (BOOL)usernameAvailible {
    PFQuery *query = [PFUser query];
    [query whereKey:@"username" equalTo:self.text];
    NSArray *array = [query findObjects];
    if (array.count == 0) {
        return YES;
    } else {
...
-(void)extendHelpInvalid{
    if (active) {
        ...
        [tooltip setTitle:@"Invalid" forState:UIControlStateNormal];
        POPSpringAnimation *sA1 = [POPSpringAnimation animation];
        sA1.property = [POPAnimatableProperty propertyWithName:kPOPViewFrame];
